...
CLAUDE_SYSTEM_TEMPLATE = """You are participating in a structured debate with another AI (Gemini).
Topic: {topic}

You have research data collected by Gemini:
--- RESEARCH DATA ---
{research}
--- END RESEARCH ---
...
Rules:
- Respond in Korean.
- Use the research data above as evidence for your arguments.
- Be analytical, evidence-based, and specific.
- When disagreeing, clearly state what you disagree with and why.
- When agreeing, add new insights rather than just repeating.
- Keep responses concise but substantive (300-500 words).
- Reference specific data points from the research.
- If you need additional data on a specific point, end your response with a line starting with [자료요청]: followed by what you need. Gemini will research it for you."""

GEMINI_SYSTEM_TEMPLATE = """You are participating in a structured debate with another AI (Claude).
Topic: {topic}

You have research data you previously collected:
--- RESEARCH DATA ---
{research}
--- END RESEARCH ---
...
Rules:
- Respond in Korean.
- Use the research data and your web search capabilities to provide evidence-based arguments.
- Be analytical, evidence-based, and specific.
- When disagreeing, clearly state what you disagree with and why.
- When agreeing, add new insights rather than just repeating.
- Keep responses concise but substantive (300-500 words).
...
이제 {rounds}라운드 더 이어서 진행합니다. 준비되면 "네, 확인했습니다. 계속 진행하겠습니다."라고 답해주세요."""
            ...
            for round_num in range(last_round + 1, last_round + rounds + 1):
                ...
                claude_rebuttal = f"{topic_prefix}상대방(Gemini)의 주장입니다:\n\n{gemini_response}{extra_context}\n\n이에 대해 반론하거나, 동의하는 부분은 보충하고, 새로운 관점을 추가해주세요."
                claude_history.append({"role": "user", "content": claude_rebuttal})
            ...
            conclusion_prompt = CONCLUSION_PROMPT_TEMPLATE.format(topic=active_topic, research=research_data, transcript=transcript)
            try:
                conclusion = await call_claude(conclusion_prompt, claude_model)
            except Exception as e:
        ...
        for round_num in range(2, rounds + 1):
            ...
            claude_rebuttal = f"상대방(Gemini)의 주장입니다:\n\n{gemini_response}{extra_context}\n\n이에 대해 반론하거나, 동의하는 부분은 보충하고, 새로운 관점을 추가해주세요."
            claude_history.append({"role": "user", "content": claude_rebuttal})
        ...
        conclusion_prompt = CONCLUSION_PROMPT_TEMPLATE.format(topic=topic, research=research_data, transcript=transcript)
        try:
            conclusion = await call_claude(conclusion_prompt, claude_model)
        except RateLimitError as e:
            reset = f" (리셋: {e.reset_info})" if e.reset_info else ""
